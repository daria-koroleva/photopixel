...
class CommentDeleteTestCase(TestSetup):
    ...
    def createComment(self, p):
        return Comment.objects.create(content = "New Comment", post = p, commenter = self.user)

    def test_commentdelete(self):
        post = self.createPost()
        com = self.createComment(post)
        response = self.client.delete('/posts/post/'+str(post.id)+'/comment/'+str(com.id)+'/')
        self.assertEqual(response.status_code, status.HTTP_200_OK)
...
class LikeCreateTestCase(TestSetup):
    ...
    def test_likecreate(self):
        ...
        post = self.createPost()
        response = self.client.post('/posts/post/' + str(post.id) + '/like', context, format='json')
        self.assertEqual(response.status_code, status.HTTP_201_CREATED)
